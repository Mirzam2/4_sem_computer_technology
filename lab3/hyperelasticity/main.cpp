...

  {
    ...
    auto mesh = std::make_shared<mesh::Mesh>(
        mesh::create_box(MPI_COMM_WORLD, {{{0.0, 0.0, 0.0}, {1.0, 1.0, 1.0}}},
                         {20, 20, 20}, mesh::CellType::tetrahedron,
    ...
    u_rotation->interpolate(
        [](auto x) -> std::pair<std::vector<T>, std::vector<std::size_t>>
        {
          ...
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            double x1 = x(1, p);
            double x2 = x(2, p);
            f(1, p) = scale * (-(x1 - x1_c) * std::cos(theta) - (x2 - x2_c) * std::sin(theta));
            f(2, p) = scale * (-(x1 - x1_c) * std::sin(theta) + (x2 - x2_c) * std::cos(theta));
          }
    ...
    auto bdofs_left = fem::locate_dofs_geometrical(
        ...
        [](auto x)
        {
          ...
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            if (std::abs(x(0, p)) < eps || std::abs(x(0, p) - 0.4) < eps)
              marker[p] = true;
          }
          return marker;
        });
    auto bdofs_right = fem::locate_dofs_geometrical(
        ...
        [](auto x)
        {
          ...
          for (std::size_t p = 0; p < x.extent(1); ++p)
          {
            if (std::abs(x(0, p) - 1) < eps || std::abs(x(0, p) - 0.6) < eps)
              marker[p] = true;
          }
          return marker;
        });
    ...
    const basix::FiniteElement S_element = basix::create_element(family, cell_type, k, discontinuous);
